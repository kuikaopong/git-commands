...
3. (Pushing a new branch) You commit some files to the `dev-foo` branch and try to "push" them to Github, but it fails as shown here:
   ...
   ```
   Explain this error.
   > In your local repository, you have created a new branch. You modify and make differently changes, finally you push it to git repository, but, on remote, Git does not create the same branch that you have created.
...
6. Consider this situation:
   ...
   > The command is rejected because the some files in the ref of the branch that you are working on, on Git repository is updated those files then pushed by another pushing. In order to fix this we need to integrate the remote changes, in another word is update or catch up the ref first.
